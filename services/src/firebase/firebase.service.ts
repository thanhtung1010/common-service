...
import {
    ...
    FIREBASE_ERROR_SERVICE,
    FIRESTORE_DEFAULT_COLLECTION,
} from './firebase.enum';
import {
    IFirebaseConfig,
    IFirestoreSearchDocument,
    IInitFirebase,
...
export class FirebaseService {
    ...
    init(option?: IInitFirebase): Observable<FirebaseApp> {
        return new Observable((subs: Subscriber<FirebaseApp>) => {
            try {
                const _option = InitFirebaseDto.fromJson(option ?? {});
                if (!this.config) {
                    ...
                    if (!this._firebaseApp) {
                        this._firebaseApp = initializeApp(
                            this.config,
                            option?.initializeAppName
                        );
                    } else {
                        console.warn(
                            'firebase app already init, system skip this step'
                        );
                    }
    ...
    initAuth(dependencies?: Dependencies): Observable<Auth> {
        return new Observable((subs: Subscriber<Auth>) => {
            try {
                if (!this._firebaseApp) {
                    throw new Error(
                        'Pls init firebase app before use this method'
                    );
                }

                if (document !== undefined) {
                    dependencies = AuthDependenciesDto.fromJson(
                        dependencies ?? {}
                    );
    ...
    checkValidService(collection: string): {
        ...
        if (!this._firebaseApp) {
            return {
                ...
                message:
                    'Firebase app have not init or firebase config is invalid, pls check your firebase console.',
            };
        }
        ...
        if (!this._auth) {
            return {
                ...
                message:
                    'Firebase auth have not init, pls run func init auth before use it.',
            };
        }
        ...
        if (!this._store) {
            return {
                ...
                message:
                    'Firebase store have not init, pls run func init store before use it',
            };
        }
    ...
    //#region method
    authStateChanged(): Observable<FirebaseUserDto | null> {
        return new Observable((subs: Subscriber<FirebaseUserDto | null>) => {
            try {
                ...
                if (
                    ...
                    onAuthStateChanged(
                        ...
                        (user) => {
                            if (user) {
                                this._user = FirebaseUserDto.fromJson(user);
    ...
    googleLogin(): Observable<FirebaseUserDto> {
        return new Observable((subs: Subscriber<FirebaseUserDto>) => {
            try {
                const checkValid = this.checkValidService('');
                if (
                    checkValid.code &&
                    ...
                    from(
                        ...
                        next: (resp) => {
                            this._user = FirebaseUserDto.fromJson(resp.user);
                            subs.next(this._user);
                            subs.complete();
                        },
                        error: (error) => {
